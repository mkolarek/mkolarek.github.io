---
layout: page
title: Curriculum Vitae
permalink: /cv/
--- 

### Experience

- **Senior Data Engineer**  
  [GROPYUS Technologies GmbH](https://gropyus.com/)   
  _Nov 2020 - present_

- **Senior Data Engineer**  
  [DAIN Studios GmbH](https://dainstudios.com/)  
  _Mar 2019 - Oct 2020_

  Senior data engineer responsible for providing technical solutions to solve business issues through the use and support of data. Leveraging machine learning and analytics use cases to build new data-driven products and services, from architecture development to implementation. Tech stack includes Amazon Web Services, Microsoft Azure, Apache Spark, Apache Kafka, Apache Hive, Python, and Scala.

- **BE2 Cohort Member**  
  [Entrepreneur First](https://www.joinef.com/)  
  _Oct 2018 - Jan 2019_
  
  Selected for Europe's most prestigious accelerator - Entrepreneur First.

- **Data Engineer**  
  [FlixMobility GmbH](https://corporate.flixbus.com/about-flixmobility/)  
  _Feb 2018 - Sep 2018_
  
  Data engineer at the Data Science Incubation team at FlixMobility. My main areas of work included developing data processing pipelines, developing and maintaining a machine learning API and basic DevOps tasks. Tech stack includes Python, Spark, Amazon Web Services, Flask, and Kubernetes.

- **Research Engineer**  
  [Zalando SE](https://jobs.zalando.com/en/)  
  _Jul 2016 - Jan 2018_
  
  Research engineer in the A/B testing and personalization team at Zalando SE. My main areas of work included performing hypothesis testing analyses, data acquisition, and cleaning and contributing to our open-source statistical analysis library. Tech stack includes Python, Spark (Scala), Amazon Web Services, Google BigQuery, and Exasol.

- **Backend Engineer**  
  [Zalando SE](https://jobs.zalando.com/en/)  
  _Sep 2015 - Jun 2016_
  
  Backend engineer in the A/B testing and personalization team at Zalando SE. My main areas of work included monitoring (on system, application and business level) and implementing parts of services and stand-alone services. Tech stack includes Scala, Python, and various Amazon Web Services.

- **Research Associate**  
  [Visage Technologies AB](https://www.visagetechnologies.com/)  
  _Jul 2014 - Jun 2015_
  
  Research associate at Visage Technologies AB (Croatian branch). My main areas of work included research and development of the eye-gazing feature of the Visage SDK, doing end-to-end work from prototyping to production. Tech stack includes OpenCV, C/C++, and Python.

- **Summer Intern**  
  [divIT systems d.o.o.](http://www.divit.hr/)  
  _Jul 2013 - Aug 2013_
  
  Software developer intern at divIT systems. Development of a module for the mash.me interactive mo-cap project (face detection and facial feature detection). Development of QR code reader with basic augmented reality features. Tech stack includes OpenCV and Unity3D.

### Education

- **MSc in Information and Communication Technology, Information Processing**  
  [Faculty of Electrical Engineering and Computing, University of Zagreb](https://www.fer.unizg.hr/)  
  _Sep 2012 - Jun 2014_
  
- **BSc in Electrical Engineering and Information Technology, Information Processing and Multimedia Systems**  
  [Faculty of Electrical Engineering and Computing, University of Zagreb](https://www.fer.unizg.hr/)  
  _Sep 2008 - Jun 2012_
